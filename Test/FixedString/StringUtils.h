...
namespace YOSEF
{
	...
	public:
		...
		static int UTF8ToUTF32(const unsigned char*src,int srcLen, YOSEFUInt32 *dst);
		static bool UTF32ToUTF8(const YOSEFUInt32*src, int srcLen, unsigned char*dst);
		static bool UTF32CharToUTF8(const YOSEFUInt32 src, unsigned char*dst, int &len);
		static bool UTF32CharToUTF16(const char32_t u32Ch, YOSEFSInt16 * u16Ch);
		static int UTF8CharLen(char ch);
		static bool IsUTF8Char(char ch);
		static int UTF82UTF32(const YOSEFUInt8 * src, YOSEFUInt32 & dst);
#ifdef _WIN32
		static int UnicodeToUTF32(const wchar_t*src, YOSEFUInt32 *dst);
