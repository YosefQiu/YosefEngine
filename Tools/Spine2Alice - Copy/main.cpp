	...
	for (int i = 0; i < skeleton->animationsCount;++i) {
		...
		for (int ti=0;ti<animation->timelinesCount;++ti){
			...
			case SP_TIMELINE_ROTATE: {
				...
				ac->set_type(Alice::kAnimationClipTypeTransform);
...
int main(int argc, char **argv) {
	...
	char szBuffer[256] = { 0 };
	sprintf(szBuffer, "%s.avatar", user_path_prefix);
	avatar.set_name(szBuffer);
	memset(szBuffer, 0, 256);
	sprintf(szBuffer, "Common/%s", user_path_prefix);
