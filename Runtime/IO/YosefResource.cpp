...
namespace YOSEF
{
	...
	std::unordered_map<YOSEFSInt32, Resource* (*)(Serializer::RuntimeAsset*)> Resource::mAssetLoader;
	Resource::Resource() :mType(ResourceTypeNone), mAssetBundle(nullptr), mResourceData(nullptr), Object(1)
	{
#if YOSEF_EDITOR
		mDescription = "Resource";
#endif
	}
	...
	Resource* Resource::LoadYOSEFAsset(const char* path, Data& data) {
		Serializer::RuntimeAsset* asset = DecodeRuntimeAsset(data);
		if (asset != nullptr) {
			std::unordered_map<YOSEFSInt32, Resource* (*)(Serializer::RuntimeAsset*)>::iterator iterDecoder = mAssetLoader.find(asset->type());
			...
			//Error("LoadYOSEFAsset:cannot decode %s",path);
		}
		return nullptr;
	}
