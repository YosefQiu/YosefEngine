...
namespace YOSEF{
	...
	public:
		...
		static Resource * LoadYOSEFAsset(const char*path, Data&data);
		...
		static std::unordered_map<YOSEFSInt32, Resource*(*)(Serializer::RuntimeAsset*)> mAssetLoader;
		template<typename T>
		static T*LoadResource(const char*path) {
			...
			if (ResourceManager::LoadData(path,data)) {
				...
				ret=(T*)LoadYOSEFAsset(path, data);
				if (ret!=nullptr){
					return ret;
				}
