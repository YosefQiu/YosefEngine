...
namespace YOSEF
{
	int StringUtils::UTF82UTF32(const YOSEFUInt8 * src, YOSEFUInt32 & dst)
	{
		int		i, iLen;
		YOSEFUInt8	b;
		b = *src++;
	...
	int StringUtils::UTF8ToUTF32(const unsigned char*src, int srcLen, YOSEFUInt32 *dst)
	{
		int currentSrcPos = 0;
		int i = 0;
    ...
    bool StringUtils::UTF32CharToUTF8(const YOSEFUInt32 src, unsigned char*dst, int &len) {
    ...
    bool StringUtils::UTF32ToUTF8(const YOSEFUInt32*src, int srcLen, unsigned char*dst)
    {
        return true;
    }
    bool StringUtils::UTF32CharToUTF16(const char32_t u32Ch, YOSEFSInt16 * u16Ch) {
	...
	int StringUtils::UnicodeToUTF32(const wchar_t*src, YOSEFUInt32 *dst)
	{
		char szTemp[256];
		memset(szTemp, 0, 256);
	...
	void StringUtils::URLEncode(const char *input, int len, char *output) {
		CURL *curl = curl_easy_init();
		if (curl) {
			char *result = curl_easy_escape(curl, input, len);
			if (result) {
	...
	int StringUtils::URLDecode(const char *input, int len, char *output) {
		CURL *curl = curl_easy_init();
		int out_len = 0;
		if (curl) {
			char *result = curl_easy_unescape(curl, input, len, &out_len);
			if (result) {
