...
namespace YOSEF{
	...
	Vector3f minVector3(const Vector3f& lhs, const Vector3f& rhs) { 
		return Vector3f(YOSEF_MIN(lhs.x, rhs.x), YOSEF_MIN(lhs.y, rhs.y), YOSEF_MIN(lhs.z, rhs.z)); 
	}
	Vector3f maxVector3(const Vector3f& lhs, const Vector3f& rhs) { 
		return Vector3f(YOSEF_MAX(lhs.x, rhs.x), YOSEF_MAX(lhs.y, rhs.y), YOSEF_MAX(lhs.z, rhs.z)); 
	}
	Vector3f Lerp(Vector3f& from, Vector3f& to, float t) { 
		return from * t + to * (1.0F - t);
	}
	Vector3f Abs(const Vector3f& v) { 
		return Vector3f(YOSEF_ABS(v.x), YOSEF_ABS(v.y), YOSEF_ABS(v.z)); 
	}
	...
	float Magnitude(const Vector3f& inV) { 
		return YOSEF_SQRTF(Dot(inV, inV));
	}
