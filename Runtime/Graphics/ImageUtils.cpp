...
namespace YOSEF{
	...
	YOSEFAny GenAlphaGradiantQuadImage(int size, float e) {
	...
	bool SaveRGBAToPNG32(const char*filePath, YOSEFUInt8*imageData, YOSEFSInt32 width, YOSEFSInt32 height){
		FIBITMAP* bitmap = FreeImage_Allocate(width, height, 32, 8, 8, 8);
		BYTE*bits = FreeImage_GetBits(bitmap);
		YOSEFUInt8*pixel = imageData;
		int dataLen = width*height;
		for (YOSEFSInt32 i = 0; i < dataLen; ++i){
			*bits++ = *pixel++;     //R
			*bits++ = *pixel++;     //G
			*bits++ = *pixel++;     //B
			*bits++ = *pixel++;     //A
	...
	bool GetImageInfo(unsigned char * const data, int dataLen, int&width, int&height, int&bpp) {
		...
		YOSEFUInt8*bits = FreeImage_GetBits(srcImage);
	...
	int RawImage2Uncompressed32BITRGBA(unsigned char * const data, int dataLen, YOSEF::Data&outPixel,int&width, int&height) {
		...
		YOSEFUInt32 bpp = FreeImage_GetBPP(srcImage);
		YOSEFUInt32 pitch = FreeImage_GetPitch(srcImage);
		if ((bits == 0) || (nWidth == 0) || (nHeight == 0))
			return 0;
		if (nWidth % 4 != 0) {
