...
namespace YOSEF
{
	...
	DynamicTextureAtlas::DynamicTextureAtlas(YOSEFUInt16 width /* = 4096 */, YOSEFUInt16 height /* = 4096 */):mMaterial(nullptr)
	{
	...
	TextureRectangleNode*DynamicTextureAtlas::AddTextureTile(YOSEFUInt64 ID, int width, int height, YOSEFUInt8 *imageData)
	{
		if (mTextureTiles.find(ID) == mTextureTiles.end())
		{
			...
			if (newNode != nullptr)
			{
				mTextureTiles.insert(std::pair<YOSEFUInt64, TextureRectangleNode*>(ID, newNode));
