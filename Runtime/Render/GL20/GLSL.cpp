...
namespace	YOSEF{
	...
	void GLSL::InitWithSerializedData(Serializer::GLSL*runtimeShader){
		...
		if (nCount == 0) {
			DefineAttribute("pos", 4, GL_FLOAT, GL_FALSE, sizeof(YOSEF::VertexDataFull), 0);
			DefineAttribute("texcoord", 4, GL_FLOAT, GL_FALSE, sizeof(YOSEF::VertexDataFull), sizeof(float) * 4);
			DefineAttribute("normal", 4, GL_FLOAT, GL_FALSE, sizeof(YOSEF::VertexDataFull), sizeof(float) * 8);
			DefineAttribute("tangent", 4, GL_FLOAT, GL_FALSE, sizeof(YOSEF::VertexDataFull), sizeof(float) * 12);
			DefineAttribute("texcoord1", 4, GL_FLOAT, GL_FALSE, sizeof(YOSEF::VertexDataFull), sizeof(float) * 16);
		}
	}
	...
	GLSL*GLSL::CreateStandardGLSL(const char*vscode, const char * fscode) {
		...
		glsl->DefineAttribute("pos", 4, GL_FLOAT, GL_FALSE, sizeof(YOSEF::VertexDataFull), 0);
		glsl->DefineAttribute("texcoord", 4, GL_FLOAT, GL_FALSE, sizeof(YOSEF::VertexDataFull), sizeof(float) * 4);
		glsl->DefineAttribute("normal", 4, GL_FLOAT, GL_FALSE, sizeof(YOSEF::VertexDataFull), sizeof(float) * 8);
		glsl->DefineAttribute("tangent", 4, GL_FLOAT, GL_FALSE, sizeof(YOSEF::VertexDataFull), sizeof(float) * 12);
		glsl->DefineAttribute("texcoord1", 4, GL_FLOAT, GL_FALSE, sizeof(YOSEF::VertexDataFull), sizeof(float) * 16);
		return glsl;
	}
	GLSL*GLSL::LoadGLSL(const char*name)
	{
		...
		if (glsl==nullptr)
		{
			...
			if (shaderResource!=nullptr)
			{
				...
				glsl->mName = name;
#if YOSEF_EDITOR
				glsl->mDescription = Resource::mAssetsIDToPathMap[name].c_str();
#endif
				mCachedGLSL.insert(std::pair<std::string, GLSL*>(name, glsl));
				if (YOSEF::StringUtils::StartWith(name, "builtin"))
				{
