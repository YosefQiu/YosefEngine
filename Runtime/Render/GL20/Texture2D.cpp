...
namespace YOSEF{
	...
	void Texture2D::InitTexture2D(YOSEFUInt16 width /* = 1024 */, YOSEFUInt16 height /* = 1024 */, 
	...
	void Texture2D::InitDepthTexture(YOSEFUInt16 width/* =1024 */, YOSEFUInt16 height/* =1024 */) {
		mWidth = width;
		mHeight = height;
	...
	void Texture2D::UploadTexture(YOSEFAny imageData){
		GlobalRenderState::BindTexture(GL_TEXTURE_2D, mTextureID);
	...
	void Texture2D::UploadETC1RGBTexture(YOSEFUInt8*imageData,YOSEFUInt16 paddedWidth, YOSEFUInt16 paddedHeight,YOSEFUInt8*alphaChannel){
#if YOSEF_ANDROID
		GlobalRenderState::BindTexture(GL_TEXTURE_2D, mTextureID);
	...
	void Texture2D::UploadSubTexture(YOSEFUInt16 x, YOSEFUInt16 y, 
		YOSEFUInt16 width, YOSEFUInt16 height, 
		GLenum format, GLenum dataType, YOSEFUInt8*pData){
		GlobalRenderState::BindTexture(GL_TEXTURE_2D, mTextureID);
	...
	Texture2D* Texture2D::CreateTextureFromMemory(const char *image_data, int image_data_len,
		...
		int bpp = RawImage2Uncompressed32BITRGBA((YOSEFUInt8*)image_data, image_data_len, data, nWidth, nHeight);
		switch (bpp)
		{
		case 32:
			break;
	...
	Texture2D*Texture2D::CreateTextureFromRawLuminance8(TextureResource*rawLuminance8){
		...
		texture2D->UploadTexture((YOSEFUInt8*)ti->imagedata().c_str());
#endif
		return texture2D;
	}
	Texture2D*Texture2D::CreateTextureFromETC1RGBFile(TextureResource*etc1rgb){
		...
		{
			YOSEFUInt8 *imageData = (YOSEFUInt8*)etc1Data.mutable_rgb()->c_str();
			YOSEFUInt16 nWidth = (imageData[12] << 8) | imageData[13];
			YOSEFUInt16 nHeight = (imageData[14] << 8) | imageData[15];
			YOSEFUInt16 paddedWidth = (imageData[8] << 8) | imageData[9];
			YOSEFUInt16 paddedHeight = (imageData[10] << 8) | imageData[11];

			Texture2D *texture2D = new Texture2D;
			texture2D->InitTexture2D(nWidth, nHeight, GL_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE, GL_ETC1_RGB8_OES, GL_RGB);
			YOSEFUInt8 *alphaData = nullptr;
			if (etc1Data.has_alpha()){
				/*alphaData = (YOSEFUInt8*)etc1Data.mutable_alpha()->c_str();
