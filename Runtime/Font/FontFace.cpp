...
namespace YOSEF {
#define YOSEF_TO_26_6_FORMAT(x) ((x)<<6)
#define YOSEF_FROM_26_6_FORMAT(x) ((x)>>6)
#define YOSEF_TO_OUTLINE_GLYPH(glyph) (reinterpret_cast<FT_OutlineGlyph>(glyph))
	...
	bool FontFace::Init(FontResource*fontData) {
		if (FT_New_Memory_Face(sFTlib, (YOSEFUInt8*)((Serializer::RuntimeAsset*)fontData->mAssetBundle)->rawdata().c_str(), ((Serializer::RuntimeAsset*)fontData->mAssetBundle)->rawdata().length(), 0, &mFace) != 0) {
	...
	float FontFace::GetLineHeight(int pt) {
		...
		if (FT_Set_Char_Size(mFace, YOSEF_TO_26_6_FORMAT(pt), YOSEF_TO_26_6_FORMAT(pt), dpix, dpiy) == 0) {
			return float(YOSEF_FROM_26_6_FORMAT(mFace->height))*mFace->size->metrics.y_scale / 65536.0f;
		}
		return 0;
	}
	FT_Glyph FontFace::GetGlyph(YOSEFUInt32 charCode, int point_size) {
		int dpix = 96;//FloorfToInt(DeviceInfo::GetDPIX());
		int dpiy = 96;//FloorfToInt(DeviceInfo::GetDPIY());
		if (FT_Set_Char_Size(mFace, YOSEF_TO_26_6_FORMAT(point_size), YOSEF_TO_26_6_FORMAT(point_size), dpix, dpiy) == 0) {
	...
	void FontFace::SetBoldSetting(FT_Glyph glyph, bool bold) {
		...
		FT_OutlineGlyph outline_glyph = YOSEF_TO_OUTLINE_GLYPH(glyph);
		if (0 != FT_Outline_Embolden(&outline_glyph->outline, YOSEF_TO_26_6_FORMAT(strength))) {
	...
	FT_Bitmap*FontFace::OutlineGlyphToBitmap(FT_Glyph glyph) {
		if (glyph->format == FT_GLYPH_FORMAT_OUTLINE) {
			...
			int charWidth = YOSEF_FROM_26_6_FORMAT(bbox.xMax - bbox.xMin);
			int charHeight = YOSEF_FROM_26_6_FORMAT(bbox.yMax - bbox.yMin);
	...
	FT_Bitmap * FontFace::GetStrokerBitMap(FT_Glyph glyph, int outline_size) {
		...
		FT_Stroker_Set(stroker, (int)(YOSEF_TO_26_6_FORMAT(outline_size)), FT_STROKER_LINECAP_ROUND, FT_STROKER_LINEJOIN_ROUND, 0);
	...
	CharInfo*FontFace::RenderingCurrentChar(YOSEFUInt32 charCode, FT_Bitmap*font, FT_Bitmap*stroker, int shadow_x, int shadow_y, YOSEF::Data&imageData) {
		...
		YOSEFUInt8*imgData = imageData.mData;
		memset(imgData, 0, imgSize);
		CharInfo *ci = new (kMemFontId)CharInfo;
		ci->mCharCode = charCode;
		if (stroker != nullptr) {
			ci->mAdvanceX = float(YOSEF_FROM_26_6_FORMAT(mFace->glyph->advance.x)) + (stroker->width - font->width);
		}
		else {
			ci->mAdvanceX = float(YOSEF_FROM_26_6_FORMAT(mFace->glyph->advance.x));
		}
		ci->mHoriBearingX = float(YOSEF_FROM_26_6_FORMAT(mFace->glyph->metrics.horiBearingX)) + font_image_offset_from_origion_x;
		ci->mHoriBearingY = float(YOSEF_FROM_26_6_FORMAT(mFace->glyph->metrics.horiBearingY)) + horizontal_offset;
		ci->mWidth = charWidth;
		ci->mHeight = charHeight;
		ci->mBaselineOffset = float(YOSEF_FROM_26_6_FORMAT(mFace->size->metrics.ascender)) + baseline_offset + float(YOSEF_FROM_26_6_FORMAT(mFace->size->metrics.descender));
		...
		for (int y = 0; y < charHeight; ++y) {
			for (int x = 0; x < charWidth; ++x) {
				...
				if (need_render_shadow) {
					...
					YOSEFUInt8 outline_value = imgData[current_offset * 4];
					YOSEFUInt8 font_value = imgData[current_offset * 4 + 3];
