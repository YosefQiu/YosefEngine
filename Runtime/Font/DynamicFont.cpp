...
namespace YOSEF{
	...
	DynamicFont::~DynamicFont() {
		for (std::unordered_map<YOSEFUInt64, CharInfo*>::iterator iter = mCachedChars.begin(); iter != mCachedChars.end(); ++iter) {
	...
	CharInfo*DynamicFont::GetCharInfoWithCharID(YOSEFUInt64 char_id) {
		std::unordered_map<YOSEFUInt64, CharInfo*>::iterator iter = mCachedChars.find(char_id);
		if (iter!=mCachedChars.end()){
			return iter->second;
		}
		return nullptr;
	}
	void DynamicFont::CheckChar(YOSEFUInt32&unicodeChar) {
		if (unicodeChar == 9) {
	...
	CharInfo*DynamicFont::NewChar(YOSEFUInt64 charID, YOSEFUInt32 unicodeChar, CharStyle style, YOSEFSInt8 size, YOSEFSInt8 outlineSize, YOSEFSInt8 shadow_x, YOSEFSInt8 shadow_y) {
		bool bold = UpdateFTMatrix(style);
		YOSEF::Data imageData;
	...
	void DynamicFont::CachingChar(CharInfo *tempChar, YOSEFUInt64 charID, YOSEFUInt8*charData){
		...
		mCachedChars.insert(std::pair<YOSEFUInt64, CharInfo*>(charID, tempChar));
	}
	bool DynamicFont::CreateChar(YOSEFUInt32 unicodeChar, CharStyle style, YOSEFSInt8 size, CharInfo**charInfo, YOSEFSInt8 outlineSize /* = 0 */, YOSEFSInt8 shadow_x /* = 0 */, YOSEFSInt8 shadow_y /* = 0 */) {
	...
	bool DynamicFont::CreateDisplayChar(const char*fontName, YOSEFUInt32 unicodeChar, CharStyle style, YOSEFSInt8 size, CharInfo**charInfo, YOSEFSInt8 outlineSize /* = 0 */, YOSEFSInt8 shadow_x /* = 0 */, YOSEFSInt8 shadow_y /* = 0 */){
