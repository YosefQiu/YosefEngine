...
#define HARD_CODED_CODE(A)  #A
	...
	uniform mat4 YOSEF_Matrix_M;
	uniform mat4 YOSEF_Matrix_V;
	uniform mat4 YOSEF_Matrix_P;
	...
	void main() {
		...
		V_Dark = texcoord1;
		V_Texcoord = texcoord;
		gl_Position = YOSEF_Matrix_P * YOSEF_Matrix_V * YOSEF_Matrix_M * pos;
	}
	);
	...
	\n#endif\n
		varying vec4 V_TexCoord;
	uniform sampler2D YOSEF_FrameBuffer;
	void main() {
		vec4 color = texture2D(YOSEF_FrameBuffer, V_TexCoord.xy);
		gl_FragColor = color;
	}
	);
