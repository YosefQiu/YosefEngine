...
lua_State*YOSEF::LuaEngine::s_GlobalStatePtr = nullptr;
int YOSEF::LuaEngine::mCurrentArgCount = 0;
...
void*YOSEFEngine_GetLuaVM() {
	return YOSEF::LuaEngine::s_GlobalStatePtr;
}
...
namespace YOSEF
{
	...
	void LuaEngine::Init() {
		...
		Info("YOSEF engine init sucess with code %d", lua_gettop(s_GlobalStatePtr));
	}
...
#if YOSEF_EDITOR || (YOSEF_WIN_PLAYER && _DEBUG)|| (YOSEF_OSX_PLAYER && _DEBUG) || DEV_PUBLISH
#else
