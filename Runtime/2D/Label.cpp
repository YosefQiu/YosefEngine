...
namespace YOSEF
{
	Label::Label(int maxCharCount){
		...
		*ret = *Material::mCachedMaterials["builtin/Material/Font"];
	...
	void Label::InitWithSerializedData(const Serializer::Label&labelData)
	{
		...
		YOSEFUInt32 temp[1024];
		memset(temp, 0, 1024 * sizeof(YOSEFUInt32));
	...
	Label::Label(const char*fontName,const YOSEFUInt32 *text, int textLen, CharStyle chStyle,int charSize,int outLineSize, Color4B color)
	{
		mDisplayText = nullptr;
		mEnd = nullptr;
		mLabelColor = color;
	...
	void Label::SetText(const UTF32Char*text, YOSEFSInt32 len){
		ClearChars();
		if (len==0){
			return;
		}
	...
	DisplayChar*Label::GetCharInfo(const UTF32Char &utf32Char, YOSEFUInt8 r /* = 255 */, YOSEFUInt8 g /* = 255 */, YOSEFUInt8 b /* = 255 */, YOSEFUInt8 a /* = 255 */) {
	...
	void Label::InsertCharAt(int nIndex, const UTF32Char &utf32Char, YOSEFUInt8 r, YOSEFUInt8 g, YOSEFUInt8 b, YOSEFUInt8 a, bool customColor) {
	...
	void Label::AppendChar(const UTF32Char &utf32Char, YOSEFUInt8 r, YOSEFUInt8 g, YOSEFUInt8 b, YOSEFUInt8 a, bool customColor) {
		DisplayChar*currentChar = GetCharInfo(utf32Char, r, g, b, a);
		if (currentChar == nullptr) {
			return;
		}
	...
	void Label::AppendCharDrawCommandAtEnd() {
		YOSEFUInt16 offset = mCharCount * 4;
		YOSEFUInt16 indexes[] = {
			offset,
			static_cast<YOSEFUInt16>(offset + 1),
			static_cast<YOSEFUInt16>(offset + 2),
			offset,
			static_cast<YOSEFUInt16>(offset + 2),
			static_cast<YOSEFUInt16>(offset + 3)
		};
		mEBO.SubData(indexes, sizeof(YOSEFUInt16) * 6, mCharCount * sizeof(YOSEFUInt16) * 6);
	...
	void Label::UpdateLocalBoundingBox(DisplayChar*currentChar) {
		...
		mLeftBottom.Set(YOSEF_MIN(vertexes[0].mVertex.x, mLeftBottom.x),
			YOSEF_MIN(vertexes[0].mVertex.y, mLeftBottom.y),
			YOSEF_MIN(vertexes[0].mVertex.z, mLeftBottom.z));
		mRightTop.Set(YOSEF_MAX(vertexes[2].mVertex.x, mLeftBottom.x),
			YOSEF_MAX(vertexes[2].mVertex.y, mLeftBottom.y),
			YOSEF_MAX(vertexes[2].mVertex.z, mLeftBottom.z));
	}
	...
	int Label::GetText(YOSEFUInt32*bufferOut)
	{
		DisplayChar*nextChar = mDisplayText;
		int i = 0;
		while (nextChar != nullptr)
	...
	*/
	...
	YOSEFSInt32 Label::GetLenCharCount()
	{
		return mCharCount;
	}
	...
	void Label::SetColor(YOSEFUInt8 r, YOSEFUInt8 g, YOSEFUInt8 b, YOSEFUInt8 a)
	{
	...
	void Label::AppendText(const char*utf8Str, YOSEFUInt8 r, YOSEFUInt8 g, YOSEFUInt8 b, YOSEFUInt8 a){
		if (strlen(utf8Str) > 0){
			UTF32Char charCode[1024] = { 0 };
			int nCount = StringUtils::UTF8ToUTF32((const unsigned char*)utf8Str, strlen(utf8Str), charCode);
			if (nCount == 0){
	...
	void Label::SetOutlineColor(YOSEFUInt8 r, YOSEFUInt8 g, YOSEFUInt8 b, YOSEFUInt8 a)
	{
	...
	void Label::SetShadowColor(YOSEFUInt8 r, YOSEFUInt8 g, YOSEFUInt8 b, YOSEFUInt8 a)
	{
