...
#include "External/LibZipRel/lib/zip.h"
#include <assert.h>
#include <errno.h>
#include <string.h>
...
static int S_archive_name_locate(lua_State* L) {
    ...
    idx = (int)zip_name_locate(*ar, fname, flags);
...
static int S_archive_stat(lua_State* L) {
    ...
    lua_pushinteger(L, (lua_Integer)(stat.index+1));
    lua_setfield(L, -2, "index");

    lua_pushnumber(L, stat.crc);
    lua_setfield(L, -2, "crc");

    lua_pushnumber(L, (lua_Number)stat.size);
    lua_setfield(L, -2, "size");

    lua_pushnumber(L, (lua_Number)stat.mtime);
    lua_setfield(L, -2, "mtime");

    lua_pushnumber(L, (lua_Number)stat.comp_size);
...
static int S_archive_set_archive_comment(lua_State* L) {
    ...
    if ( 0 != zip_set_archive_comment(*ar, comment, (zip_uint16_t)comment_len) ) {
        lua_pushstring(L, zip_strerror(*ar));
        lua_error(L);
    }
...
static int S_archive_set_file_comment(lua_State* L) {
    ...
    if ( 0 != zip_get_file_comment(*ar, path_idx, comment, comment_len) ) {
        lua_pushstring(L, zip_strerror(*ar));
        lua_error(L);
    }
...
static int S_archive_rename(lua_State* L) {
    ...
    if ( NULL != path ) {
        path_idx = (int)zip_name_locate(*ar, path, 0);
        if ( path_idx < 0 ) {
...
static int S_archive_file_read(lua_State* L) {
    ...
    len = (int)zip_fread(*file, buff, len);
