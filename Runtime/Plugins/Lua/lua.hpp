...
#define EXPORT_NAME(N) "Yosef."#N
#define METATABLENAME(T) "MT_"#T

#if YOSEF_WIN_PLAYER || YOSEF_WIN_EDITOR
#define YOSEF_MODULE(X) YOSEF::##X
#else
#define YOSEF_MODULE(X) YOSEF::X
#endif
...
#define EXPORT_API_NEW {"New", New_ExportedYOSEFType},
...
#define SAFE_INVOKE_LUA_METHOD_ARG0(m)do{\
    if(m!=LUA_REFNIL){lua_State *L = YOSEF::LuaEngine::Store();lua_getref(L, m);\
	if(lua_isfunction(L,-1)){if (lua_pcall(L, 0, 0, -2) != 0){ Error("%s",lua_tostring(L, -1));}\
}YOSEF::LuaEngine::Restore();}}while(0)
#define SAFE_INVOKE_LUA_CLASS_METHOD_ARG0(t,m) do{\
	lua_State *L = YOSEF::LuaEngine::Store();lua_getref(L, t);\
...
#define SAFE_INVOKE_LUA_METHOD_ARG1F(m,arg1)do{\
	lua_State *L = YOSEF::LuaEngine::Store();lua_getref(L, m);\
...
#define SAFE_INVOKE_LUA_CLASS_METHOD_2F_1I(t,m,a1,a2,a3) do{\
	if(m!=LUA_REFNIL){\
		lua_State *L = YOSEF::LuaEngine::Store();\
		lua_getref(L, m);\
		lua_getref(L, t);\
		lua_pushnumber(L, a1);\
		lua_pushnumber(L, a2);\
		lua_pushinteger(L, YOSEFSInt32(a3));\
...
#define DEF_YOSEF_EXPORTED_TYPE_NEW_METHOD(cpp_type,luatype) static int New_ExportedYOSEFType(lua_State*L) \
{\
